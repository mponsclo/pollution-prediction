...
from src.anomaly.detector import (
    train_anomaly_pipeline, predict_anomalies, evaluate_anomaly_detection,
)
...
def train_anomaly(target: dict):
    ...
    with mlflow.start_run(run_name=f"anomaly_{sc}_{name}"):
        mlflow.log_params({
            "station_code": sc, "item_code": ic, "pollutant": name,
            "model_type": "lgbm_classifier", "n_features": 80,
            "approach": "supervised", "xgbod_pattern": True,
        })
        ...
        del pipe, final_pipe; gc.collect()
